            ...
            {isLoggedIn ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-cyan-500/20 to-violet-500/20 border border-cyan-500/30 text-cyan-300 text-sm hover:from-cyan-500/30 hover:to-violet-500/30 transition-colors"
                >
                ...
                {showUserProfile && user && (
                  <div 
                    className="absolute right-0 top-full mt-2 w-72 bg-slate-900 rounded-xl border border-cyan-500/30 shadow-xl z-50 p-4"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {/* Avatar et nom */}
                    <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-700">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-r from-cyan-500 to-violet-500 flex items-center justify-center text-white font-bold">
                        {user.name?.[0]?.toUpperCase() || user.email[0].toUpperCase()}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-white">{user.name || 'Belle Âme'}</div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      <div className="text-center p-2 rounded-lg bg-slate-800/50">
                        ...
                        <div className="text-sm font-bold text-white">{user.credits}</div>
                        <div className="text-[10px] text-slate-400">Crédits</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-slate-800/50">
                        <Star className="w-4 h-4 text-amber-400 mx-auto mb-1" />
                        <div className="text-sm font-bold text-white">{user.loyaltyPoints}</div>
                        <div className="text-[10px] text-slate-400">Points</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-slate-800/50">
                        <Zap className="w-4 h-4 text-violet-400 mx-auto mb-1" />
                        <div className="text-sm font-bold text-white">Nv.{user.level}</div>
                    ...
                    {user.promoCodes && user.promoCodes.length > 0 && (
                      <div className="mb-4 p-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        <div className="text-xs text-amber-300 flex items-center gap-1">
                          ...
                          <span>Code: <strong>{user.promoCodes[0].code}</strong> (-{user.promoCodes[0].discount}%)</span>
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      ...
                      className="w-full py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm hover:bg-red-500/20 transition-colors"
                    >
                      Se déconnecter
                    </button>
                  </div>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-cyan-500 to-violet-500 text-white text-sm font-medium shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all"
              >
        ...
        <div className="flex-shrink-0 px-4 py-2 border-t border-cyan-500/10 bg-slate-900/30">
          <div className="flex items-center justify-center gap-2 flex-wrap">
            ...
            <QuickEmailButton 
              ...
              label="📧 Récap"
            />
            <ReferralButton onClick={() => setShowReferral(true)} />
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-sm bg-slate-900 rounded-2xl border border-cyan-500/20 overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={toggleSound}
                  className={`w-10 h-6 rounded-full transition-colors ${soundEnabled ? 'bg-cyan-500' : 'bg-slate-700'}`}
                >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`w-10 h-6 rounded-full transition-colors ${reminderEnabled ? 'bg-cyan-500' : 'bg-slate-700'}`}
                >
              ...
              <div className="pt-4 border-t border-slate-700">
                ...
                <button
                  ...
                  className="w-full py-2 mb-3 rounded-lg bg-gradient-to-r from-cyan-500/20 to-violet-500/20 border border-cyan-500/30 text-cyan-300 text-sm flex items-center justify-center gap-2"
                >
