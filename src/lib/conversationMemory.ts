  ...
  try {
    ...
    if (!memory) {
      ...
      return {
        ...
        interests: newMemory.interests,
        commonQuestions: newMemory.commonQuestions,
        favoriteServices: newMemory.favoriteServices,
        purchaseHistory: newMemory.purchaseHistory,
        ...
        designHumain: newMemory.designHumainType ? {
          ...
          authority: newMemory.designHumainAuthority || ''
        } : undefined,
        lunarPhase: newMemory.lunarPhase || undefined,
        overallSentiment: newMemory.overallSentiment as MemoryData['overallSentiment'],
        recentSentiments: newMemory.recentSentiments as MemoryData['recentSentiments'] || []
      };
    }
    ...
    return {
      ...
      commonQuestions: memory.commonQuestions,
      favoriteServices: memory.favoriteServices,
      purchaseHistory: memory.purchaseHistory,
      ...
      designHumain: memory.designHumainType ? {
        ...
        authority: memory.designHumainAuthority || ''
      } : undefined,
      lunarPhase: memory.lunarPhase || undefined,
      overallSentiment: memory.overallSentiment as MemoryData['overallSentiment'],
      recentSentiments: memory.recentSentiments as MemoryData['recentSentiments'] || []
    };
  } catch (error) {
...
export async function updateUserMemory(
  ...
  try {
    ...
    if (updates.commonQuestions) updateData.commonQuestions = updates.commonQuestions;
    if (updates.favoriteServices) updateData.favoriteServices = updates.favoriteServices;
    if (updates.purchaseHistory) updateData.purchaseHistory = updates.purchaseHistory;
    if (updates.birthDate !== undefined) updateData.birthDate = updates.birthDate || null;
    if (updates.lunarPhase !== undefined) updateData.lunarPhase = updates.lunarPhase || null;
    if (updates.overallSentiment) updateData.overallSentiment = updates.overallSentiment;
    if (updates.recentSentiments) updateData.recentSentiments = updates.recentSentiments;
    ...
    if (updates.designHumain) {
      ...
      updateData.designHumainAuthority = updates.designHumain.authority;
    }
    ...
    await prisma.conversationMemory.upsert({
      ...
      create: {
        ...
        commonQuestions: updates.commonQuestions || [],
        favoriteServices: updates.favoriteServices || [],
        purchaseHistory: updates.purchaseHistory || [],
        ...
        designHumainAuthority: updates.designHumain?.authority || null,
        lunarPhase: updates.lunarPhase || null,
        overallSentiment: updates.overallSentiment || 'neutral',
        recentSentiments: updates.recentSentiments || []
      },
      update: updateData
    });
  } catch (error) {
